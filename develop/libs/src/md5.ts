/* Нашёл отличное решение для работы с md5, переписал его под typescript */
...
export class Md5_ts{
     ...
     */
    private static raw_md5(s): string {
    ...
    private static raw_hmac_md5(k, d): string {
        return this.rstr_hmac_md5(this.str2rstr_utf8(k), this.str2rstr_utf8(d));
    }

    private static hex_hmac_md5(k, d): string {
        return this.rstr2hex(this.raw_hmac_md5(k, d));
    }

    public static md5(string: string, key?: string, raw?: string): string {
        if (!key) {
            if (!raw) {
